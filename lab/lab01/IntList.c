...
void IntListInsertInOrder(IntList L, int v)
{
	...
	// When the list if empty.
    if (L->first == NULL) { 
        L->first = new;
	    L->last = new;
	} else {	
	    
	    //	Find the correct position the new should insert.    
	    while (n != NULL && n->data <= v) {
	        pre = n;
	        n = n->next;
	    }
	    
	    // When v is the largest value.
	    if (n == NULL && pre != NULL) { 
	        pre->next = new;
	        L->last = new;	      
	    } 
	    
	    // When v is the smallest value.
	    else if (pre == NULL) {         	    
	        new->next = L->first; 
	        L->first = new;	       
	    } 
	    
	    // When v is somewhere in the middle.
	    else { 
	        pre->next = new;
	        new->next = n;	      
	    }
